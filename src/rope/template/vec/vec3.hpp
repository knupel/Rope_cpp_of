...
public:
  ...
  vec3 & y(T const &y);
	...
	vec3 & rand(vec3<T> const &max, std::default_random_engine &generator);
	vec3 & rand(T const &x_min, T const &y_min, T const &z_min, 
							T const &x_max, T const &y_max, T const &z_max,
...
template <class T>
T vec3<T>::mag_sq(vec3<T> const &v) const {
...
double vec3<T>::mag() const {
	return ::sqrt(vec3<T>::mag_sq());
}

template <class T>
double vec3<T>::mag(vec3<T> const &v) const {
	return ::sqrt(vec3<T>::mag_sq(v));
}
...
template <class T>
vec3<T>	& vec3<T>::rand(vec3<T> const &max) {
	return rand(0, 0, 0, 
							max.x(), max.y(), max.z());
}
...
template <class T>
vec3<T>	& vec3<T>::rand(vec3<T> const &max, std::default_random_engine &generator) {
	return rand(0, 0, 0,
							max.x(), max.y(), max.z(),
							generator);
}
	...
	switch(vec<T>::get_type()) {
		case 'c':
			...
			if(std::is_same<T, uint8_t>::value) {
				...
				this->z(random_char(static_cast<uint8_t>(z_min), static_cast<uint8_t>(z_max), generator));
			}
			if(std::is_same<T, unsigned char>::value) {
				...
				this->z(random_char(static_cast<unsigned char>(z_min), static_cast<unsigned char>(z_max), generator));
			}
			break;
